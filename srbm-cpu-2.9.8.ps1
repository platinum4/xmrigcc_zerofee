...
$Cfg = ReadOrCreateMinerConfig "Do you want use to mine the '$Name' miner" ([IO.Path]::Combine($PSScriptRoot, $Name + [BaseConfig]::Filename)) @{
	...
	Algorithms = @(
		...
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "xelishashv2" }
...
#                if ("argon2id_chukwa2" -contains $_.Algorithm) { $_.Algorithm = "argon2id_chukwa" }
                if ("xelishash" -contains $_.Algorithm) { $_.Algorithm = "xelishashv2" }
...
#				if ("randomyada" -contains $_.Algorithm -or $Pool.Name -notmatch "mrr") {
#                [MinerInfo]@{
#					Pool = $Pool.PoolName()
#					PoolKey = $Pool.PoolKey()
#					Priority = $Pool.Priority
...
#					Path = "$Name\SRBMiner-MULTI.exe"
#					ExtraArgs = $extrargs
#					Arguments = "--algorithm randomepic;$($_.Algorithm) --pool 51pool.online:3416;$pools --wallet platinum4#1x5070-7950X3D;$($Pool.User) --password epiccash;$($Pool.Password) --multi-algorithm-job-mode 3 --tls false;$tls --api-enable --api-port $port --miner-priority 1 --disable-gpu --disable-worker-watchdog --retry-time $($Config.CheckTimeout) $nicehash $extrargs"
#					Port = $port
#					BenchmarkSeconds = if ($_.BenchmarkSeconds) { $_.BenchmarkSeconds } else { $Cfg.BenchmarkSeconds }
#					RunBefore = $_.RunBefore
#					RunAfter = $_.RunAfter
