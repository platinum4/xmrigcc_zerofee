	...
	if ($_.Enabled) {
		...
		if ($Algo -and $AlgoDual) {
			...
			if ($Pool -and $PoolDual) {
				...
				$fee = 1
				if (("alephium", "etchash", "ethash", "ironfish") -contains $_.Algorithm) { $fee = 0.7 }
				...
				[MinerInfo]@{
					...
					Path = "$Name\rigel.exe"
					ExtraArgs = $extrargs
					Arguments = "-a $($_.Algorithm)+$($_.DualAlgorithm) $hosts -w $([Config]::WorkerNamePlaceholder) --api-bind 127.0.0.1:$port --no-strict-ssl --no-watchdog --stats-interval 60 --dag-reset-mclock off $nocolor$extrargs"
					Port = $port
