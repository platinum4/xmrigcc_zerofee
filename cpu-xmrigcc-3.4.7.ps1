...
$Cfg = ReadOrCreateMinerConfig "Do you want use to mine the '$Name' miner" ([IO.Path]::Combine($PSScriptRoot, $Name + [BaseConfig]::Filename)) @{
	...
	Algorithms = @(
		...
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "rx/tuske" }
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "rx/wow" }
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "rx/xdag" }
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "rx/xeq" }
		...
		if ($Algo) {
			...
			if ($Pool) {
				...
				[MinerInfo]@{
					...
					ExtraArgs = $extrargs
					Arguments = "-a $($_.Algorithm) $pools -R $($Config.CheckTimeout) --http-port=4045 --cpu-priority 0 $extrargs"
					Port = 4045
