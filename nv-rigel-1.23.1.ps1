	...
	if ($_.Enabled) {
		...
		if ($Algo) {
			...
			if ($Pool) {
				...
				$fee = 1
				if ($_.Algorithm -match "xelishash") { $_.Algorithm = "xelishashv3" }
				elseif (("alephium", "etchash", "ethash") -contains $_.Algorithm) { $fee = 0.7 }
				elseif (("nexapow", "octopus", "xelishash", "xelishashv2", "xelishashv3") -contains $_.Algorithm) { $fee = 2 }
				...
				[MinerInfo]@{
					...
					Path = "$Name\rigel.exe"
					ExtraArgs = $extrargs
					Arguments = "-a $($_.Algorithm) $hosts -u $($Pool.User) -p $($Pool.Password) --api-bind 127.0.0.1:$port --dns-over-https --no-strict-ssl --no-watchdog --stats-interval 60 --dag-reset-mclock off $extrargs"
					Port = $port
                ...
                elseif ($Pool.Name -notmatch "nicehash") {
				[MinerInfo]@{
					Pool = $Pool.PoolName()
					PoolKey = $Pool.PoolKey()
					Priority = $Pool.Priority
					...
					Path = "$Name\rigel.exe"
					ExtraArgs = $extrargs
					Arguments = "-a $($_.Algorithm) $hosts -u $($Pool.User) -p $($Pool.Password) -w $([Config]::WorkerNamePlaceholder) --api-bind 127.0.0.1:$port --dns-over-https --no-strict-ssl --no-watchdog --stats-interval 60 --dag-reset-mclock off $extrargs"
					Port = $port
					BenchmarkSeconds = if ($_.BenchmarkSeconds) { $_.BenchmarkSeconds } else { $Cfg.BenchmarkSeconds }
					RunBefore = $_.RunBefore
					RunAfter = $_.RunAfter
