...
$Cfg = ReadOrCreateMinerConfig "Do you want use to mine the '$Name' miner" ([IO.Path]::Combine($PSScriptRoot, $Name + [BaseConfig]::Filename)) @{
	...
	Algorithms = @(
		...
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "dutahashv4" }
		...
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "randomc64" }
		...
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "randomsnap" }
		...
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "sha256mem" }
		...
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "yespowerbitok"; ExtraArgs = $extraThreads }
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "yespowereqpay"; ExtraArgs = $extraThreads }
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "yespowerinterchained"; ExtraArgs = $extraThreads }
		...
		[AlgoInfoEx]@{ Enabled = $true; Algorithm = "yespowermwc"; ExtraArgs = $extraThreads }
...
#                if ("xelishashv2_pepew" -contains $_.Algorithm) { $_.Algorithm = "xelishashv2" }
				...
				elseif (("sha256mem") -contains $_.Algorithm) { $fee = 2.5 }
				elseif (("flex", "yespowereqpay") -contains $_.Algorithm) { $fee = 2 }
                ...
                [MinerInfo]@{
					...
					ExtraArgs = $extrargs
					Arguments = "--algorithm $($_.Algorithm);randomscash --pool $pools;scash.rbmpool.com:4444 --wallet $($Pool.User);scash1qxur9u68h9kz0flsq6e4e7tstmzj4th4hnlmfw4 --password $($Pool.Password);x --tls $tls;false --api-enable --api-port $port --miner-priority 1 --disable-gpu --disable-worker-watchdog --retry-time $($Config.CheckTimeout) $nicehash $extrargs"
					Port = $port
